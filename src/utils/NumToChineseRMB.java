...
public class NumToChineseRMB {
    ...
    public static void main(String[] args) {
        System.out.println("\n--------将数字转换成中文金额的大写形式--------\n");
        NumToChineseRMB trans = new NumToChineseRMB();
        String s = trans.cleanZero(trans.splitNum(trans.roundString(trans.getNum())));
        // 如果转换过后是一个空串，则不输出屏幕
        if (!"".equals(s)) {
    ...
    /**
     * 从命令行接收一个数，在其中调用checkNum()方法对其进行验证，并返回相应的值
     *
    ...
    /**
     * 把用户输入的数以小数点为界分割开来，并调用numFormat()方法进行相应的中文金额大写形式的转换
     ...
     * 把传入的数转换为中文金额大写形式
     *
     * @param flag int 标志位，1表示转换整数部分，0表示转换小数部分
     ...
     * 注：传入的这个数应该是经过splitNum()方法进行处理，这个字符串应该已经是用中文金额大写形式表示的
     *
    ...
    private String cleanZero(String s) {
        ...
        // 如果用户开始输入了很多0，去掉字符串前面多余的'零'，使其看上去更符合习惯
        while (s.charAt(0) == '零') {
            // 将字符串中的 "零" 和它对应的单位去掉
            s = s.substring(2);
        ...
        // 第二轮转换考虑 "零亿","零万","零元"等情况，"亿","万","元"这些单位有些情况是不能省的，需要保留下来
        for (int i = 0; i < 3; i++) {
